...
function addMarker(lat, lng, label, count, country) {
...
function renderSidebar(countries) {
  ...
  sidebar.innerHTML = countries.map(function (c) {
    return '<div class="card" data-country="' + c.country + '" onclick="focusCountry(\'' + c.country + '\')"><div class="flex items-center gap-3"><span style="font-size:20px">' + (emoji[c.country] || '\u{1F30D}') + '</span><div><div class="font-semibold text-sm">' + c.country + '</div><div class="text-xs text-muted">' + c.count + ' active events</div></div></div></div>';
  }).join('');
}
...
(async function loadMap() {
  ...
  document.querySelector('.section-header p').textContent = total + ' active events across ' + countries.length + ' countries';
  initMap([20, 0], 2);
  for (const c of countries) {
    addMarker(c.lat, c.lng, c.country, c.count, c.country);
  }
