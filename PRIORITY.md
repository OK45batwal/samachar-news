...
| P-03 | | feature | Bookmarks page shows empty state even when signed in. No folder management UI despite `folder` column existing in DB. | `frontend/bookmarks.html` |
...
| ID | Status | Type | What | Where |
|----|--------|------|------|-------|
| P-05 | | polish | Seed data reuses same 4 demo articles across 23 entries, 5 countries. Add country-specific content. | `backend/seed_e2e_inline.py` |
| P-06 | | polish | `difflib.SequenceMatcher` fuzzy country matching runs O(n²) per `/geo` request. Replace with `get_close_matches` or pre-indexed names. | `backend/routes/news.py:70-99` |
...
| P-10 | | feature | WebSocket `subscribe`/`unsubscribe` handled in loop but never acted on. Either implement per-channel or remove handlers. | `backend/websocket/ws.py:96-98` |
| P-11 | | infra | CI skips Vite build — e2e serves raw HTML. Add `npm run build` step before e2e to catch build failures. | `.github/workflows/test.yml` |
